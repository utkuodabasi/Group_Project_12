...
- the "Entire Residential Home" is the base category and it has significant an intercept of 7.7 which implies that this type of property drives the price of the accommodation positively by USD 7.7.
- the property type "Entire Villa" has a significant intercept of 0.14. By adding the intercept of the base category we obtain an intercept of 7.83 which implies that this type of properties drives the price of the accommodation by a relative move of USD 7.83.
- the property type "Farm stay" has a absolute significant intercept of 7.392 which implies that this type of properties drives the price of the accommodation by a relative move of USD 7.39.
- the property type "Private room in a farm stay" has an absolute significant intercept of 6.834 which implies that this type of property drives the price by a relative move of USD 6.834
...
When we investigate VIFs, we see that none of them is higher than 5. Although, "bedrooms" variable is close to 5 with a VIF value of 4.7. So these could potentially be co-linear variables. 
...
```
Since there wasn't a clear way to group neighborhoods qualitatively, we grouped the neighborhood_cleansed data according to the number of hosts in each neighborhood and listed the remaining as others. 
...
```{r}
model_final <- lm(log(price_4_nights)~prop_type_simplified + number_of_reviews+ room_type+accommodates+host_is_superhost+instant_bookable+neighbourhood_simplified+availability_30+reviews_per_month,data = listings)
...
- By midnight on Monday 17 Oct 2021, you must upload on Canvas a short presentation (max 4-5 slides) with your findings, as some groups will be asked to present in class. You should present your Exploratory Data Analysis, as well as your best model. In addition, you must upload on Canvas your final report, written  using R Markdown to introduce, frame, and describe your story and findings. You should include the following in the memo:
